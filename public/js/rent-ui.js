...
window.FPH.rentUI = (() => {
  ...
  async function submitPayment() {
    ...
    try {
      if (method === 'paystack') {
        const email = window.prompt('Enter your email for the Paystack checkout');
        if (!email) return;
        const result = await FPH.rent.initializePaystackPayment(invoiceId, { email, amount: payload.amount });
        if (result?.data?.authorizationUrl) {
          window.open(result.data.authorizationUrl, '_blank', 'noopener,noreferrer');
          FPH.toast.success('Paystack checkout opened. Complete the payment to confirm your invoice.', 'Paystack');
