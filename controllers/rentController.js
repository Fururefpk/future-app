...
exports.initializePaystackPayment = async (req,res,next) => {
  try {
    ...
    if (!invoiceId) return err(res,'Invoice id is required',400);
    if (!amount || Number(amount) <= 0) return err(res,'amount is required',400);
    if (!email) return err(res,'email is required',400);

    const inv = await Invoice.findById(invoiceId);
    if (!inv) return err(res,'Invoice not found',404);
    if (inv.status === 'voided') return err(res,'Cannot pay a voided invoice',400);
    ...
    ok(res, {
      ...
      reference,
      invoice: inv,
      paystack: { publicKey: paystack.getPaystackConfig().publicKey },
    }, 200);
  } catch (e) {
    next(e);
  }
...
exports.handlePaystackWebhook = async (req,res,next) => {
  try {
    ...
    if (!paystack.isValidWebhookSignature(payload, signature)) {
      return err(res,'Invalid Paystack signature',401);
    }
    ...
    if (event?.event === 'charge.success' && event?.data?.status === 'success') {
      ...
      if (invoiceId) {
        ...
        if (inv && inv.status !== 'voided') {
          ...
          const existingPayment = inv.payments.find((p) => p.transactionRef === reference);
          if (!existingPayment) {
            inv.payments.push({
              method: 'card',
